...
class ThoughtBox:
    def __init__(self, database_path: str, explicitly_create_tables:bool = False):
        ...
        if create_tables:
            ...
            cur.execute( "CREATE TABLE tags (number INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT UNIQUE)")
    ...
    def listTags(self) -> List[Tag]:
        cur = self.conn.cursor()
        tags = [] 
        for row in cur.execute("SELECT group_concat(DISTINCT tags.title) as title FROM tags"):
            tags.extend( [  Tag.fromStr(t.strip()) for t in row[0].split(',') if t.strip() ])
        return tags
    ...
    def listThoughts(
        ...
        names: List[Name] = [],
        tags: List[Tag] = [],
        linked_to: List[str] = [], print_query=False
    ) -> List[Thought]:
        """
        ...
        queries = []
        if len(tags) > 0:
            tagQuery = " OR ".join( [ f"tags.title='{sql_escape(str(t.title))}'" for t in tags ])
        ...
        if len(linked_to) > 0:
            linkQuery = " OR ".join( [ f"links.target={sql_escape(str(l))}" for l in linked_to ])
        ...
        query = (f"SELECT tbl.number, tbl.title, group_concat(DISTINCT tags.title), group_concat(DISTINCT links.target) "
        "FROM "
        ...
        for row in cur.execute(query):
            ...
            link_bits = [ Link(source=number, target=l.strip()) for l in row[3].split(",") if l.strip() ]
            thoughts.append( Thought(name=row[0], title=row[1], tags=tag_bits, links=link_bits, content=[], sources=[]) )
    ...
    def listThoughtsByTag(
        ...
        thoughts = self.listThoughts(names=names,tags=tags,linked_to=linked_to,print_query=print_query)
    ...
    def addOrUpdate(self, thought: Thought):
        ...
        cur = self.conn.cursor()
        cur.execute(f"INSERT INTO thoughts (number, title) VALUES ('{str_name}', '{sql_escape(thought.title)}')")

        if len(thought.links) > 0:
            linkValues = ", ".join( [ f"('{sql_escape(str(l.source))}', '{sql_escape(str(l.target))}')" for l in thought.links ])
            cur.execute("INSERT INTO links (source, target) VALUES "+linkValues)


        if len(thought.tags) > 0:
            tagValues = ", ".join( [ f"('{sql_escape(str(t.title))}')" for t in thought.tags ])
            cur.execute(f"INSERT INTO tags (title) VALUES {tagValues} ON CONFLICT DO NOTHING")
            self.conn.commit()

            cur = self.conn.cursor()
            tagQuery = " OR ".join( [ f"(title='{sql_escape(str(t.title))}')" for t in thought.tags ])
    ...
    def rename(self, name: Name, new_name: Name) -> None:
        ...
        cur = self.conn.cursor()
        cur.execute(f"UPDATE thoughts SET number='{str_new_name}' WHERE number='{str_name}'")
        cur.execute(f"UPDATE tag_links SET thought='{str_new_name}' WHERE thought='{str_name}'")
        cur.execute(f"UPDATE links SET target='{str_new_name}' WHERE target='{str_name}'")
        cur.execute(f"UPDATE links SET source='{str_new_name}' WHERE source='{str_name}'")
        self.conn.commit()
